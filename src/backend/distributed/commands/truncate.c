...
/* DisabledTriggerListStack management */

/*
 * DisabledTriggerListStack stores a list for each truncate command execution.
 * Each list in stack consists of DisabledTriggerData pointers for each disabled
 * truncate trigger during execution of that truncate command.
 */
List *DisabledTriggerListStack = NIL;

static void PushEmptyDisabledTriggerList(void);
static void DisableTruncateTriggersOnTableIfNecessary(Oid relationId);
static void AddDisabledTriggerToCurrentList(Oid relationId, Oid disabledTriggerId,
											char previousEnableDisableState);
static List * PopCurrentDisabledTriggerList(void);

typedef struct DisabledTriggerData
{
	Oid relationId;
	Oid disabledTriggerId;
	char previousEnableDisableState;
} DisabledTriggerData;
...
PreprocessTruncateStatement(TruncateStmt *truncateStatement)
{
	...
	DisableTruncateTriggersIfNecessary(truncateStatement);
}


/*
 * DisableTruncateTriggersIfNecessary disables explicit truncate triggers for
 * each relation being truncated with truncateStatement by executing
 * DisableTruncateTriggersOnTableIfNecessary.
 */
void
DisableTruncateTriggersIfNecessary(TruncateStmt *truncateStatement)
{
	/*
	 * Push an empty list to store DisabledTriggerData pointers that might be
	 * registered by DisableTruncateTriggersOnTableIfNecessary.
	 */
	PushEmptyDisabledTriggerList();

	RangeVar *relation = NULL;
	foreach_ptr(relation, truncateStatement->relations)
	{
		bool missingOk = true;
		Oid relationId = RangeVarGetRelid(relation, ShareRowExclusiveLock, missingOk);
		DisableTruncateTriggersOnTableIfNecessary(relationId);
	}
}


/*
 * PushEmptyDisabledTriggerList pushes an empty list to DisabledTriggerListStack
 * in order to store DisabledTriggerData for each disabled truncate trigger for
 * the execution of current truncate command.
 */
static void
PushEmptyDisabledTriggerList()
{
	DisabledTriggerListStack = lcons(NIL, DisabledTriggerListStack);
}


/*
 * DisableTruncateTriggersOnTableIfNecessary disables explicit truncate triggers
 * for the relation with relationId if it is a citus local table. Then saves
 * DisabledTriggerData for each disabled trigger to re-enable them via
 * RevertTruncateTriggersEnableDisableStates.
 */
static void
DisableTruncateTriggersOnTableIfNecessary(Oid relationId)
{
	/*
	 * Relation may not exist as standard process utility is not executed yet.
	 * Let postgres to error out by just returning in below two checks.
	 */
	...
	{
		/* we already don't allow triggers on other citus table types */
		relation_close(relation, NoLock);
		return;
	}

	List *triggerIdList = GetExplicitTriggerIdList(relationId);
	List *truncateTriggerIdList = FilterTriggerIdListByEvent(triggerIdList,
															 TRIGGER_TYPE_TRUNCATE);

	Oid truncateTriggerId = InvalidOid;
	foreach_oid(truncateTriggerId, truncateTriggerIdList)
	{
		HeapTuple heapTuple = GetTriggerTupleById(truncateTriggerId);
		Assert(HeapTupleIsValid(heapTuple));
		Form_pg_trigger triggerForm = (Form_pg_trigger) GETSTRUCT(heapTuple);

		bool skipSystemTriggers = true;
		EnableDisableTrigger(relation,
							 NameStr(triggerForm->tgname),
							 TRIGGER_DISABLED,
							 skipSystemTriggers,
							 lockMode);

		char previousEnableDisableState = triggerForm->tgenabled;
		AddDisabledTriggerToCurrentList(relationId, truncateTriggerId,
										previousEnableDisableState);
	}

	/* make the changes done by EnableDisableTrigger effective in current transaction */
...
/*
 * AddDisabledTriggerToCurrentList appends DisabledTriggerData to the current
 * list that stores disabled triggers for the current truncate command execution.
 */
static void
AddDisabledTriggerToCurrentList(Oid relationId, Oid disabledTriggerId,
								char previousEnableDisableState)
{
	DisabledTriggerData *disabledTriggerData = palloc0(sizeof(DisabledTriggerData));
	disabledTriggerData->relationId = relationId;
	disabledTriggerData->disabledTriggerId = disabledTriggerId;
	disabledTriggerData->previousEnableDisableState = previousEnableDisableState;

	/* append DisabledTriggerData for the current truncate command execution */
	List **currentDisabledTriggerList = (List **) &linitial(DisabledTriggerListStack);
	*currentDisabledTriggerList = lappend(*currentDisabledTriggerList,
										  disabledTriggerData);
}


/*
 * RevertTruncateTriggersEnableDisableStates pops the current list that stores
 * DisabledTriggerData for the triggers disabled by last executed truncate
 * command and reverts their enable states back.
 */
void
RevertTruncateTriggersEnableDisableStates()
{
	List *currentDisabledTriggerList = PopCurrentDisabledTriggerList();

	DisabledTriggerData *disabledTriggerData = NULL;
	foreach_ptr(disabledTriggerData, currentDisabledTriggerList)
	{
		Oid relationId = disabledTriggerData->relationId;
		Oid triggerId = disabledTriggerData->disabledTriggerId;
		char previousEnableDisableState = disabledTriggerData->previousEnableDisableState;
...
/*
 * PopCurrentDisabledTriggerList pops current list that stores DisabledTriggerData
 * from DisabledTriggerListStack.
 */
static List *
PopCurrentDisabledTriggerList()
{
	List *currentDisabledTriggerList = linitial(DisabledTriggerListStack);
	DisabledTriggerListStack = list_delete_first(DisabledTriggerListStack);
	return currentDisabledTriggerList;
}
