...
/*
 * EnsureConnectionPossibilityForLocalNode tries to reserve a shared
 * connection counter for the local node. The exceptions are:
 *  - Reservation is not prohibited (see IsReservationPossible())
 *  - there is at least one connection to the node so that we are guranteed
 *    to get a connection
 *  - An earlier call already reserved a connection (e.g., we allow only a
 *    single reservation per backend)
 */
bool
CheckConnectionPossibilityForLocalNode(void)
{
	if (!IsReservationPossible())
	{
		return false;
	...
	bool found = false;
	ReservedConnectionHashEntry *hashEntry =
		AllocateOrGetReservedConectionEntry(localHostName, localPort,
											userId, MyDatabaseId, &found);
	...
	/*
	 * If there is space for one connection to the local host, we are good and can
	 * safely mark that we have a reserved connection.
	 */
	if (TryToIncrementSharedConnectionCounter(localHostName, localPort))
	{
		/* locally mark that we have one connection reserved */
		hashEntry->usedReservation = false;
