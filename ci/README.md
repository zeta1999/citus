...
These are replacements are only available on Windows normally. We make most of
them available with this header file:
```c
...
This script is tries to make sure that we don't add useless declarations to our
...
before running this script, make sure that that you've indeed the case.
...
#endif
```
This was deemed to error prone and not worth the effort.
